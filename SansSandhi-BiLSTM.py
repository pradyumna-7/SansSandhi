...
class BiLSTMTagger(nn.Module):
    def __init__(self, vocab_size, label_size, embedding_dim=64, hidden_dim=128):
        super().__init__()
...
if __name__ == "__main__":
    path = "data/dataset.txt"  # Input file
    ...
    model = BiLSTMTagger(len(char2idx), len(label2idx))
...
def run_inference():
    ...
    model = BiLSTMTagger(len(char2idx), len(idx2label))
    model.load_state_dict(torch.load("sandhi_model.pt", map_location=torch.device('cpu')))
    model.eval()
