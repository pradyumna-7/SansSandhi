...
class CNNBiLSTMTagger(nn.Module):
    def __init__(self, vocab_size, label_size, embedding_dim=64, hidden_dim=128, cnn_out=128):
        ...
        # BiLSTM
        self.lstm = nn.LSTM(cnn_out, hidden_dim // 2, batch_first=True, bidirectional=True)
    ...
    def forward(self, x):
        ...
        out = self.relu(self.conv1(cnn_in))  # (B, C, L)
        out = self.relu(self.conv2(out))  # (B, C, L)
        out = self.relu(self.conv3(out))  # (B, C, L)
        out = self.dropout(out)

        out = out.permute(0, 2, 1)  # (B, L, C)
        lstm_out, _ = self.lstm(out)  # (B, L, H)
        logits = self.fc(lstm_out)  # (B, L, label_size)
        return logits
...
if __name__ == "__main__":
    ...
    model = CNNBiLSTMTagger(len(char2idx), len(label2idx)).to(DEVICE)
...
def run_inference():
    ...
    model = CNN_BiLSTM_Tagger(len(char2idx), len(idx2label)).to(DEVICE)
